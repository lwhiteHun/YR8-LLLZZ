...
function productStat() {
    var topMax = product[0].sold;
    var topProductId = 0;
    var newestMax = product[0].createdAt;
    var newestProduct = product[0].name;
    var sumPrice = 0;
    for (var i in product) {
        if (product[i].sold > topMax) {
            topMax = product[i].sold;
            topProductId = i;
        }
        if (product[i].createdAt > newestMax) {
            newestMax = product[i].newestMax;
            newestProduct = product[i].name;
        }
        sumPrice += product[i].price;
    }
    console.log(`Legnépszerűbb: ${product[topProductId].name}, eladott mennyiség: ${topMax} db`);
    console.log(`Legújabb: ${newestProduct}`);
    console.log(`Összérték: ${sumPrice} Ft`);
    console.log(`Átlagár: ${Math.round(sumPrice / product.length)} Ft`);
}
